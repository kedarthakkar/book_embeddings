...
def interaction_df_to_book_vectors(interaction_df: pd.DataFrame, book_to_index: dict[int, int]) -> dict[int, list[float]]:
    ...
    book_embedding_df['publication_decade'] = pd.cut(book_embedding_df['publication_year'], bins=DECADE_BINS,
                                                     labels=DECADE_LABELS, right=False)
...
def get_sparse_tensor(interaction_df: pd.DataFrame, user_to_index: dict[int, int], book_to_index: dict[int, int]) -> tuple[torch.Tensor, torch.Tensor]:
    def get_label_id(book_ids):
        return [book_to_index[book_id] for book_id in book_ids]

    grouped_interactions = interaction_df[['user_id', 'book_id']].groupby('user_id')['book_id'].apply(get_label_id).reset_index()
    sparse_tensor = torch.zeros(len(user_to_index), len(book_to_index), dtype=torch.float)
